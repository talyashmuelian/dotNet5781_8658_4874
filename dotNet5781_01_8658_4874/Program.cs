...
namespace dotNet5781_01_8658_4874
{
				...
				ch = int.Parse(chStr);
					switch (ch)
					{
							...
							for (var i = 0; i < listBuses.Count; i++)//מעבר על כל הרשימה לחיפוש האוטובוס המבוקש Go through the entire list to search for the requested bus
						{
								...
								if (listBuses[i].checkIfReady(r)==true)//לזמן מתודה שתבדוק האם האוטובוס הנמצא יכול לצאת לנסיעה מבחינת דלק וקילומטרז, או שעברה שנה מאז הטיפול האחרון'
										  {
												listBuses[i].doingDriving(r);//לזמן מתודה שמוסיפה את הקילומטרים של נסיעה זו For a method time that adds the miles of this trip
								}
									else
    ...
    class Bus
    {
		...
		private double kilometersFromTreament;//מספר הקילומטרים מאז הטיפול האחרון Number of miles since last treatment
		private double kilometersUntilLastTreatment;//נסועה בה בוצע הטיפול האחרון A ride in which the last treatment was performed
		private double kilometraj;//נסועה כוללת Total travel
		private double kilometers;//כמות הקילומטרים מאז התדלוק The amount of miles since refueling
		...
		public void setKilometers (double num) { kilometers = num; }
        ...
        public bool checkIfReady(double kilimeterForTravel)//מתודה שבודקת אם אוטבוס יכול לצאת לנסיעה מסוימת A method that checks if a bus can go on a particular trip
		{
			DateTime date1 = DateTime.Now;
			if (int(date1- dateTreatLast)>365)//עברה שנה מאז הטיפול האחרון It has been a year since the last treatment
				return false;
			double sum = kilimeterForTravel + kilometersFromTreament;//סכום הקילומטרים שהיו מאז הטיפול האחרון פלוס הנסיעה שהוגרלה The amount of miles that have been since the last treatment plus the raffle ride
		...
		public void doingDriving(double kilimeterForTravel)
        {
